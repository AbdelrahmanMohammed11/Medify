from ..VectorDataBaseInteface import VectorDataBaseInterface
from ..VectorDataBaseEnums import (DistanceMethodEnums, PgVectorTableSchemeEnums, 
                                   PgVectorIndexTypeEnums, VectorDataBaseEnums)

import logging
from typing import List
from models.DB_Schema import RetrieveDocument
from sqlalchemy.sql import text as sql_text
import json

class PGVector(VectorDataBaseInterface):
    
    def __init__(self, db_client,
                 defualt_vector_size:int = 786,
                 distance_method: str = None, index_threshold :int = 1500):
        
        self.db_client = db_client
        self.defualt_vector_size=defualt_vector_size
        self.index_threshold = index_threshold
        if distance_method == 'cosine':
            distance_method = DistanceMethodEnums.COSIN.value
    ...
    async def get_collection_info(self, collection_name:str) -> dict:
        async with self.db_client() as session:
            async with session.begin():
                ...
                rec_count = await session.execute(count_sql)
                ...
                return {
                    ...
                    "record_count": rec_count.scalar_one(),
                }
    ...
    async def create_vector_index(self, 
        ...
        async with self.db_client() as session:
            async with session.begin():
                ...
                rec_counts = results.scalar_one()
                
                if rec_counts < self.index_threshold:
                    return False
                
                self.logger.info(f"START: Create vector index for Collection: {collection_name}")
    ...
    async def reset_vector_index(self, collection_name: str,
        ...
        async with self.db_client() as session:
            async with session.begin():
                drop_idx = sql_text(f'DROP INDEX IF EXISTS {index_type}')
                await session.execute(drop_idx)
    ...
    async def insert_one(self,
        ...
        is_collection_existed = await self.is_collection_existed(collection_name=collection_name)
        if not is_collection_existed:
            self.logger.info(f"can't insert new record for non-existed collection: {collection_name}")
            return False
        
        if not record_id:
            self.logger.error(f"Can't insert new record without chunk_id")
            return False
    ...
    async def insert_many(self,
        ...
        # Some Validation befor creating records
        ...
        if len(vectors) != len(record_ids):
            self.logger.info(f"can't insert new records number of vectors not equals to the number of chunk_ids Collection: {collection_name}")
            return False
        
        if len(metadatas) == 0 or not metadatas:
    ...
    async def search_by_vector(self, collection_name, vector, limit):
        ...
        # validate that the collection(Table) is exists 
        is_collection_existed = await self.is_collection_existed(collection_name=collection_name)
        if not is_collection_existed:
            self.logger.info(f"can't search for records for non-existed collection: {collection_name}")
            return False
