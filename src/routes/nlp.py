...
async def index_project(request: Request, project_id: int, push_request: PushRequest):
    ...
    if not project:
        return JSONResponse(
            ...
            content={
                "signal": ResponseStatus.ResponseSignal.value
            }
        )
    ...
    while has_records:
        ...
        if not is_inserted:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={
                    "Error":ResponseStatus.INSERT_INTO_DB_ERROR.value
                }
            )
    ...
    return JSONResponse(
            content={
                    "signal":ResponseStatus.INSERT_INTO_DB_Done.value,
                    "Inserted Items_counts": inserted_items_count
                }
            )
